...
// --- KDS PANEL COMPONENT & LOGIC (EMBEDDED FOR THIS VIEW) ---
...
const PosView: React.FC = () => {
   ...
   } = useApp(); // [HYDRA V1.0 UPGRADE]
   const currentShift = useStore(state => state.dailyData?.currentShift);

   // [HYDRA V1.1 HOTFIX] --- ARC ---
   // Dynamically build class names for the main view wrapper
   const posViewClasses = [
       'pos-view',
   ...
   return (
       <main className={posViewClasses}>
           <div className={`pos-view-overlay ${isOrderPanelOpen ? 'is-visible' : ''}`} onClick={() => setIsOrderPanelOpen(false)}></div>
           {!currentShift && <ShiftStartOverlay />}
           
           {/* Pass 'collapsed' class to child components */}
